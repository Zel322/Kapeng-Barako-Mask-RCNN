...
class MeanAveragePrecisionCallback(Callback):
    def __init__(self, train_model, inference_model, dataset,
                 calculate_map_at_every_X_epoch=5, target_mAP=0.8, dataset_limit=None,
                 verbose=1):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, target_mAP, logs=None):
        ...
        if epoch > 2 and (epoch+1)%self.calculate_map_at_every_X_epoch == 0:
            ...
            if mAP < self.target_mAP: 
                print("mAP is less than target mAP value (mAP={0})".format(self.target_mAP))
            else:
                self.stopped_epoch = epoch
                print("mAP has exceeded the target mAP value (mAP={0}). Stopping training...".format(self.target_mAP))
